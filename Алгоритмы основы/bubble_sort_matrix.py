...
def b_sort_matrix(matrix):
    total_steps = 0
    total_time = 0
    total_comparisons = 0

    for row in matrix:
        time_taken, steps, comparisons = bubble_sort(row)
    ...
    for col in range(num_columns):
        ...
        for row in range(num_rows):
            current_column.append(matrix[row][col])

        time_taken, steps, comparisons = bubble_sort(current_column)
        total_time += time_taken
        total_steps += steps
        total_comparisons += comparisons
...
time_spent, steps, comparisons = b_sort_matrix(randommatrix)
